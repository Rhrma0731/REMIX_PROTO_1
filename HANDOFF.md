...
> 마지막 업데이트: 2026-03-04 (아이템 아이콘 연결, 몬스터 스프라이트 교체, 신규 아이템 406~410, PassiveTrigger 버그 수정)
...
| 범위 | 분류 | 상태 |
|------|------|------|
| 101~110 | Form 무기 아이템 | ✅ 아이콘 연결 완료 |
| 201~210 | Body 파츠 아이템 | ✅ 아이콘 연결 완료 |
| 301~310 | Head 파츠 아이템 | ✅ (아이콘 미연결) |
| 401~410 | 전설/일반 아이템 | ✅ 401~405 전설 / **406~410 신규 추가** (아이콘 미연결) |
| 501~507 | 특수 신규 | ✅ 메뉴 생성 (아이콘 미연결) |
| 601~610 | 패밀리어 소환 신규 아이템 | ✅ 메뉴 생성 (아이콘 미연결) |
...
| 프리팹 | 색상 | 모드 | 사용 아이템 | 상태 |
|--------|------|------|-------------|------|
| `Familiar_Bee.prefab` | 노란 원 `#FFD900` | Orbit | 기본 Orbit 패밀리어 | ❌ 삭제됨 |
...
- [ ] **ItemData 스프라이트 연결** — Form(101~110), Body(201~210) 아이콘 연결 완료. Head(301~310) / 전설(401~405) / 특수(501~507) / 패밀리어(601~610) 아이콘 아직 null
...
| 항목 | 설명 | 심각도 |
|------|------|--------|
| AppearanceSprite 일부 없음 | Form(101~110)/Body(201~210)는 연결 완료. Head/전설/특수/패밀리어 계열은 아직 null | 낮음 |
...
│       ├── Monster3.png                    ← 몬스터 스프라이트 (교체 신규, Golem 대체)
│       ├── monster1.png                    ← 몬스터 스프라이트 (미배정)
│       ├── StickyHand_Sprite.png           ← StickyHand 적 스프라이트
│       ├── PROTOTYPE_1.png                 ← 프로토타입 참고 이미지
│       └── Item_Icon/                      ← ✅ 아이템 아이콘 (101~110, 201~210 — 20종)
│           ├── 101.png ~ 110.png           ← Form 무기 아이템 아이콘
│           └── 201.png ~ 210.png           ← Body 파츠 아이템 아이콘
├── Resources/
│   ├── Familiars/
...
| 2026-03-04 | 아이템 아이콘 20종 추가 — `Assets/Graphic/2D graphic/Item_Icon/` (101~110, 201~210) |
| 2026-03-04 | Form(101~110), Body(201~210) 에셋 Icon/AppearanceSprite/FormWeaponSprite/PedestalSprite 연결 완료 |
| 2026-03-04 | Monster3.png 추가 / Golem_Weak_Mob_2_Walk_NE.png 삭제 (몬스터 스프라이트 교체) |
| 2026-03-04 | 신규 아이템 5종 추가 — 406(깔래/ST_SLOW), 407(매운맛 떡꼬치/ST_BURN), 408(등딱 스매싱 주먹), 409(반짝이는 짱돌), 410(식당 이쑤시개) |
| 2026-03-04 | Familiar_Bee.prefab 삭제 / 아이템 에셋 정리 (510, 601~602 삭제) |
| 2026-03-04 | fix: ItemEffectRunner ActiveItemPipeline.Initialize() — Modifier/Action 먼저 초기화하도록 순서 변경, PassiveTrigger 즉시 발동 시 패밀리어 미소환 버그 수정 |
